...
list_of_all_irfs <- readRDS(results_file)[[1]]
...
if (length(list_of_all_irfs) != length(countries_modeled)) {
  nmin <- min(length(list_of_all_irfs), length(countries_modeled))
  list_of_all_irfs  <- list_of_all_irfs[seq_len(nmin)]
  countries_modeled <- countries_modeled[seq_len(nmin)]
  cat("⚠️ Longitudes no coincidían; se alinearon por el mínimo común.\n")
}
names(list_of_all_irfs) <- countries_modeled
cat("Nombres de países asignados. ✅\n")
...
cat("\n--- 2. VISUALIZACIÓN (GRÁFICO DE VIOLÍN) ---\n\n")
...
compare_groups_bayesian <- function(draws_df, g1_filter, g2_filter, g1_name, g2_name, target = c("mcap","vtrd")) {
  ...
  cat(sprintf("• Probabilidad de que el impacto acumulado promedio (%s, 1–3) sea MAYOR en '%s' que en '%s': %.2f%%\n",
              toupper(target), g1_name, g2_name, 100*prob))
  invisible(prob)
}
...
prob_impacto_pos_mcap <- cumulative_draws %>%
  ...
  filter(!is.na(grupo)) %>%
  group_by(grupo) %>%
  summarise(`P(impacto > 0) [%] (MCAP, 1–3)` = round(mean(avg > 0, na.rm = TRUE)*100, 2), .groups = "drop")

cat("Probabilidad de que el impacto acumulado sea POSITIVO por grupo (MCAP, horizontes 1–3):\n")
...
prob_impacto_pos_vtrd <- cumulative_draws %>%
  ...
  filter(!is.na(grupo)) %>%
  group_by(grupo) %>%
  summarise(`P(impacto > 0) [%] (VTRD, 1–3)` = round(mean(avg > 0, na.rm = TRUE)*100, 2), .groups = "drop")

cat("\nProbabilidad de que el impacto acumulado sea POSITIVO por grupo (VTRD, horizontes 1–3):\n")
